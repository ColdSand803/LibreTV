...
export function Header(props: { showSearch?: boolean }) {
  return (
    <Suspense fallback={<header className="fixed top-0 left-0 right-0 h-16 bg-[#141414] z-40" />}>
      <HeaderContent {...props} />
    </Suspense>
  );
}
    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-8 h-16 flex items-center gap-6">
          ...
          >
            <span className="text-[#E50914] font-black text-2xl sm:text-3xl tracking-tighter uppercase font-sans select-none drop-shadow-[0_2px_10px_rgba(229,9,20,0.5)] group-hover:scale-105 transition-transform">
              LIBREFLIX
            </span>
          </Link>
          ...
          <div className="flex items-center gap-3">
            {showSearch && (
              <div
                ...
                className={cn(
                  ...
                  searchExpanded
                    ? 'w-48 sm:w-64 bg-black/80 border border-white/40 rounded-full px-3 py-1'
              ...
              >
                ...
                <input
                  ref={searchInputRef}
                  className={cn(
                    'bg-transparent text-white text-sm focus:outline-none ml-2 transition-all',
                ...
                {searchHistory.visible && searchExpanded && (
                  <div className="absolute top-11 right-0 w-72">
            ...
            <button
              ...
              aria-label="设置与采集源管理"
            >
              <div className="w-8 h-8 rounded-md bg-gradient-to-tr from-[#E50914] to-red-400 flex items-center justify-center text-white font-bold text-xs shadow-md border border-white/20 group-hover:ring-2 group-hover:ring-white transition-all overflow-hidden">
                <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-white translate-y-0.5">
              ...
              <span className="hidden sm:inline text-xs text-zinc-400 group-hover:text-white transition-colors">
                ▼
              </span>
            </button>
          </div>
